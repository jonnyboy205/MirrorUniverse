...
public class CopyOfG4Player implements Player {
	...
	public int lookAndMove(int[][] aintViewL, int[][] aintViewR) {
		...
		if (rightExitSet && leftExitSet) {
			...
			// Cannot have this as an else case, it may need to be executed as well as the previous statement
			if (path.isEmpty()) {
	...
	private class OurPoint extends Point implements Comparable{
		private int dist;
		private ArrayList<Integer> thePath;
		...
		public OurPoint(int x, int y, int currX, int currY, int player){
			...
			AStar_Single as = new AStar_Single(currX, currY, x, y, tempMap);
			as.setMaxNodes(radius * 50);
			Node_Single ns = as.findPath();
			if(ns == null){
				as.setMaxNodes(radius * 100);
				as.findPath();
			}
			if(ns == null){
				dist = Integer.MAX_VALUE;
			} else {
				thePath = ns.getActionPath();
				dist = thePath.size();
			}
		}
		...
		public ArrayList<Integer> getThePath(){
			return thePath;
		}
			...
			for (int i=p1Pos[1]; i<kb_p1.length; i++){
				for (int j=p1Pos[0]; j<kb_p1.length; j++){
					...
					if (kb_p1[i][j] == 0 && checkSurroundingCellsForFives(1, i, j) == true){
						points.add(new OurPoint(j, i, p1Pos[0], p1Pos[1], 1));
					}
				}
			...
			for (int i=p2Pos[1]; i<kb_p2.length; i++){
				for (int j=p2Pos[0]; j<kb_p2.length; j++){
					...
					if (kb_p2[i][j] == 0 && checkSurroundingCellsForFives(2, i, j) == true){
						points.add(new OurPoint(j, i, p2Pos[0], p2Pos[1], 2));
					}
				}
