...
public class State {
	...
	public static boolean keepAlignment;
	...
	public int exploreNext(byte[] move, State[] nextState) {
		...
		for (int d = 1 ; d != 9 ; ++d) {
			...
			if (keepAlignment && (immLeft || immRight))
				continue;
			if (!immLeft) {
				nextState[c].i1 = (byte) (i1 + di);
				nextState[c].j1 = (byte) (j1 + dj);
			...
			if (leftNowOnGoal || rightNowOnGoal)
				continue;
			move[c++] = (byte) dir;
		}
	//	System.out.println(this + " -> ");
	//	for (int i = 0 ; i != c ; ++i)
	//		System.out.println(nextState[i]);
		return c;
	}
	...
	public int unknownNeighbors() {
		...
		for (int d = 1 ; d != 9 ; ++d) {
			...
			if (!immLeft && leftMap[nextState.i1][nextState.j1] == -1)
				c++;
			if (!immRight && rightMap[nextState.i2][nextState.j2] == -1)
				c++;
		}
		return c;
	}
